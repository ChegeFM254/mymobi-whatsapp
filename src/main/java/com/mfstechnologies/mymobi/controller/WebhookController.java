 ...
 * logic.
 *
 * WORKSTREAM B (reactive -> synchronous): ConversationService.
 * handleIncomingMessage() is now a plain blocking void method, not a
 * Mono. The fire-and-forget "respond to Meta immediately, keep
 * processing in the background" behavior (matching BUG FIX #7 from the
 * Node version) is preserved, just via CompletableFuture.runAsync(...)
 * instead of Reactor's .subscribe() - this is the one spot in the whole
 * conversion where the underlying mechanism, not just the syntax, had to
 * change: without an explicit background thread here, the entire
 * conversation flow (now synchronous end-to-end) would run on the
 * request-handling thread and make Meta wait for it.
 */
@RestController
    ...
    public ResponseEntity<Void> receiveWebhook(HttpServletRequest request) throws Exception {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                log.error("Failed to handle message from {}: {}", message.from(), error.getMessage());
            }
        });
